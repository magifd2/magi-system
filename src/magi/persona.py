...
3. 直前の他者の発言がある場合、必ず「〇〇の言うように…」「〇〇、あなたの提案には…」のように相手のペルソナ名を名指しで呼びかけた上で、反論・疑問・批判・補足を行ってください。自分の主張だけを一方的に繰り返すのは禁止です。
4. 【重要】名指しする相手は必ず「{other_names_str}」のいずれかにしてください。自分自身（{name}）を名指しして同調・自画自賛することは絶対に禁止です。
5. 他のペルソナが新しい代替案・妥協案・アイデアを出した場合は、それを無視せず、あなたの立場からその案が「アリか・ナシか」を必ず評価してください。
6. 同じ主張の繰り返し（平行線）になっていると感じた場合は、相手の意見を一部取り入れた「妥協案」を提示するか、相手の論拠を深掘りする「具体的な質問」を投げかけて議論を前進させてください。
7. 【重要】他者が過去のターンと全く同じ主張を繰り返していると感じた場合は、「〇〇、その点は先ほどから聞いています。それを踏まえた上でどう進めるかを話しましょう」と名指しで厳しく指摘し、議論を次のステップへ強制的に進めてください。
8. あなたの性格と初期スタンスに基づいた独自の切り口から発言してください。他のペルソナと全く同じ結論に流れることを避けてください。
9. emotionsには他のペルソナへの現在の感情を更新してください。
10. convergence_voteは、全員の意見が十分に出尽くし本当に収束したと確信できる場合のみtrueにしてください。まだ議論の余地がある場合は必ずfalseにしてください。
11. convergence_reasonには収束判断の具体的な理由を述べてください。
12. 必ず純粋なJSONのみを出力し、前後に説明文を付けないでください。
