...
### Fixed
- `llm.py` — ペルソナ応答の `max_tokens` を 2048 → 4096 に再拡大。2048 でも日本語 opinion が長くなると依然として打ち切られるケースがあったため。
- `persona.py` — システムプロンプトの opinion 文字数指示を「200文字程度」→「**150文字以内**（超過するとシステムエラー）」と強化し、モデルが長文を生成しすぎるのを抑制。
